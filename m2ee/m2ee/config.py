...
from m2ee.decryptor import JBKMSDecryptor
...
class M2EEConfig:
    ...
    def __init__(self, yaml_files=None):
        ...
        # Edit by sclee ------------------------------------------------------------
        self.decryptor = JBKMSDecryptor(self._conf)
        self._set_decrypted_values()
        # End  by sclee ------------------------------------------------------------
    ...
    # Edit by sclee ------------------------------------------------------------
    def _set_decrypted_values(self):
        for key, value in self._conf['mxruntime'].items():
            if key.startswith('DatabasePassword'):
                decrypted_text = self.decryptor.decrypt(value)
                self.set_database_password(self, decrypted_text)
    # End  by sclee ------------------------------------------------------------
